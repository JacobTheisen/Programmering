...
function test() {
  connection.query("SELECT * FROM Leaderboard", (error, results) => {
    ...
    for (let player of results) {
      console.log(player.score + " " + player.id + " " + player.email);
    }
  });
} */
...
console.log(leaderboardArray[0].score);

//leaderboardList.innerHTML += leaderboardArray.username + ": " + leaderboardArray.score;
    }
leaderboardArray.sort(function (a, b) {return b.score - a.score});
//JSON.stringify(leaderboardArray);
  });
  for (var i = 0; i < 5; i++) {
 //leaderboardList.innerHTML += leaderboardArray[i].username + ": " + leaderboardArray[i].score;
 ...
 loggInnKnapp.onclick = () => {
  ...
  connection.query('SELECT * FROM Leaderboard', (error, results) => {
    ...
    for (let spiller of results) {
      if (spiller.username == brukernavn.value) {
...
function visSaldo() {
  ...
  connection.query('UPDATE Leaderboard SET score=? WHERE username=?', [(parseInt(saldo) / 1000) * 100, brukernavn.value]);
}
